...
function BigNum.HyperE(val, digit: number?): string
	val = BigNum.convert(val)
	local man, exp = val[1], val[2]
	local newExp = math.floor(math.log10(exp))
	local lfe = math.fmod(exp, 3)
	exp /= 10^newExp
	man = BigNum.showDigits(man* 10^ lfe, digit)
	exp = BigNum.showDigits(exp*10 ^lfe, digit)
...
function BigNum.lbencode(val): number
	...
	local compress = (math.log10(exp+1)+1)*4503599627370496
	return compress
end
...
function BigNum.lbdecode(val: number): BigNum
	...
	val = math.abs(val)
	local exp = 10^(val/4503599627370496-1)-1
	local man = val/((math.log10(exp+1)+1) * 4503599627370496)
	return BigNum.new(man, exp)
end
